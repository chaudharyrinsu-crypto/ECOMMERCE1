...
import Nav from '../mens-components/Nav'
                    ...
                    .map(item => (
                        <div key={item.id} className='bg-white rounded-2xl p-4 relative hover:z-50 hover:scale-[1.055] hover:shadow-xl duration-200 group'>
                            ...
                            <div className='grid grid-cols-5 gap-2 duration-200 opacity-0 group-hover:opacity-100 bg-white absolute group-hover:z-50 group-hover:shadow-xl p-4 left-0 right-0 w-full rounded-b-2xl'>
                                {item.sizes.map(size => (
                                    <Link href='/' key={size} className='border border-[#E0DACF] flex items-center justify-center text-sm p-2 tracking-tighter cursor-pointer'>{size}</Link>
                                ))}
                            </div>
                        </div>
                    ))
