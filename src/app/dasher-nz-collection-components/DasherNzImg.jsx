...
const DasherNzImg = () => {
    return (
        <>
            <section className='grid grid-cols-1 md:grid-cols-2 gap-2.5 text-white pt-10'>
                {dasherNzShoe.map((item,index) => (
                    <div key={index} className='relative text-center'>
                        <img className='rounded-2xl' src={item.image} alt="dasherImg" />
                        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 space-y-5 md:space-y-8'>
                            <h2 className='text-3xl lg:text-4xl'>{item.title}</h2>
                            <p className=''>{item.description}</p>
                            <div className='py-2 flex gap-2 whitespace-nowrap uppercase text-sm font-medium text-[#212121]'>
                                <Link className='bg-white px-5 md:px-12 text-center py-2 rounded-4xl border hover:bg-[#212121] hover:text-white border-none duration-500 block' href='/'>Shop men</Link>
                                <Link className='bg-white px-5 md:px-12 text-center py-2 rounded-4xl border hover:bg-[#212121] hover:text-white border-none duration-500 block' href='/'>Shop women</Link>
                            </div>
                        </div>
                    </div>
                ))}
